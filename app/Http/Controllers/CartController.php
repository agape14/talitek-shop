...
class CartController extends Controller
{
    ...
    public function addToCart(Request $request){
        ...
        if (empty($request->slug)) {
            request()->session()->flash('error','Productos no válidos');
            return back();
        }        
        ...
        if(!auth()->check()) {
            return back()->with('error', 'Debe iniciar sesión antes de agregar un producto.');
        }
        ...
        if($already_cart) {
            ...
            if ($already_cart->product->stock < $already_cart->quantity || $already_cart->product->stock <= 0) return back()->with('error','¡Stock insuficiente!.');
            $already_cart->save();
            ...
            if ($cart->product->stock < $cart->quantity || $cart->product->stock <= 0) return back()->with('error','¡Stock insuficiente!.');
            $cart->save();
            $wishlist=Wishlist::where('user_id',auth()->user()->id)->where('cart_id',null)->update(['cart_id'=>$cart->id]);
        }
        request()->session()->flash('success','El producto ha sido añadido al carrito.');
        return back();       
    }  
    ...
    public function cartDelete(Request $request){
        ...
        if ($cart) {
            $cart->delete();
            request()->session()->flash('success','Carrito eliminado exitosamente');
            return back();  
        }
        request()->session()->flash('error','Error por favor inténtalo de nuevo');
        return back();       
    }     
    ...
    public function cartUpdate(Request $request){
        ...
        if($request->quant){
            ...
            return back()->with('Carrito inválido!');
        }    
    }
